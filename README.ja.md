...
AutoHotkeyやPowerToys、マクロキーボードで目的を達成でき、コードの管理にも困っていないなら、無理に乗り換える必要はありません。RELYRは、通常の入力を残したまま、キー・マウス・ジェスチャー・アプリ別プロファイル・DeckをひとつのAction一覧から画面で変更したい場合に向いています。

まずは次の小さな設定で試せます。

- Spaceを押しながらEでエクスプローラーを開く：Spaceレイヤー、Eキー、「エクスプローラーを開く」を順に選び、保存します。
- 作業用Deckを作る：Deckエディターを開き、空き枠へEXEやショートカットをドロップし、Windows操作やPCモニターのActionを追加して保存します。

どちらも全面的な移行は不要で、既存の通常入力を保ったまま試せます。
...
アンインストール時は自動起動を解除し、CapsLockのF13割り当てを標準へ戻します。通常のアンインストールでは、ユーザーデータを残すか完全に削除するか確認します。`winget uninstall`を含む無人アンインストールでは、意図しないデータ消失を防ぐため、`%AppData%\RELYR`の設定と`%LocalAppData%\RELYR`の診断情報を標準で残します。無人実行で完全に削除する場合は、インストール済みのアンインストーラーを`/VERYSILENT /PURGEUSERDATA=1`付きで実行してください。

## 利用条件

RELYRはMIT Licenseで提供します。インストールを続行する前に、安全な設定、無保証、責任制限を簡潔にまとめた多言語の[利用条件](TERMS.md)を表示します。同意済みの通常アップデートでは、同じ条件を繰り返し表示しません。
