...
internal static partial class UpdateService
{
    ...
    internal static (Version Version,string VersionText) ParseLatestVersion(string json)
    {
        ...
        if(root.TryGetProperty("draft",out var draft)&&draft.GetBoolean())throw new InvalidDataException("公開済みの更新情報を確認できませんでした。");
        if(root.TryGetProperty("prerelease",out var prerelease)&&prerelease.GetBoolean())throw new InvalidDataException("公開済みの更新情報を確認できませんでした。");
        if(!root.TryGetProperty("tag_name",out var tagElement))throw new InvalidDataException("更新情報にバージョンがありません。");
        string versionText=tagElement.GetString()?.Trim().TrimStart('v','V')??"";
        if(!Version.TryParse(versionText,out var version))throw new InvalidDataException("更新情報のバージョンを読み取れませんでした。");
