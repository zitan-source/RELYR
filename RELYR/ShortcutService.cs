...
internal static class ShortcutService
{
    ...
    {
        macroName = "";
        if (args.Count < 2 || !args[0].Equals("--run-macro-base64", StringComparison.OrdinalIgnoreCase))
            return false;
        try
        {
            macroName = Encoding.UTF8.GetString(Convert.FromBase64String(args[1]));
    ...
    {
        macroId = "";
        if (args.Count < 2 || !args[0].Equals("--run-macro-id", StringComparison.OrdinalIgnoreCase))
            return false;
        macroId = args[1].Trim();
    ...
    static string CreateShortcut(string macroName, string arguments, string? destinationDirectory, string? executablePath)
    {
        if (string.IsNullOrWhiteSpace(macroName))
            throw new ArgumentException("マクロ名が空です。", nameof(macroName));
        ...
        string fileName = string.Concat(macroName.Trim().Select(x => Path.GetInvalidFileNameChars().Contains(x) ? '_' : x));
        ...
        object shell = Activator.CreateInstance(shellType) ?? throw new InvalidOperationException("Windowsショートカット機能を開始できません。");
        object? shortcut = null;
        try
        {
            shortcut = shellType.InvokeMember("CreateShortcut", System.Reflection.BindingFlags.InvokeMethod, null, shell, [shortcutPath]) ?? throw new InvalidOperationException("ショートカットを作成できません。");
            ...
            type.InvokeMember("Arguments", System.Reflection.BindingFlags.SetProperty, null, shortcut, [arguments]);
            type.InvokeMember("WorkingDirectory", System.Reflection.BindingFlags.SetProperty, null, shortcut, [Path.GetDirectoryName(executable) ?? ""]);
            type.InvokeMember("Description", System.Reflection.BindingFlags.SetProperty, null, shortcut, [$"RELYR マクロ: {macroName}"]);
            type.InvokeMember("IconLocation", System.Reflection.BindingFlags.SetProperty, null, shortcut, [MacroIconLocation(executable)]);
    ...
    {
        if (string.IsNullOrWhiteSpace(oldName) || oldName.Equals(macro.Name, StringComparison.Ordinal))
            return null;
        string directory = destinationDirectory ?? Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory);
        string oldPath = ExistingShortcutPath(oldName, directory);
    ...
    {
        string directory = destinationDirectory ?? Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory);
        string path = ExistingShortcutPath(macro.Name, directory);
        if (!File.Exists(path))
            return null;
        string upgraded = CreateMacroShortcut(macro, directory, executablePath);
        if (!path.Equals(upgraded, StringComparison.OrdinalIgnoreCase))
            File.Delete(path);
        return upgraded;
    }
    ...
    {
        string current = ShortcutPath(macroName, directory);
        if (File.Exists(current))
            return current;
        string fileName = string.Concat(macroName.Trim().Select(x => Path.GetInvalidFileNameChars().Contains(x) ? '_' : x));
        if (string.IsNullOrWhiteSpace(fileName))
            fileName = "Input Customizer マクロ";
