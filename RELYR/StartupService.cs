...
public static class StartupService
{
    ...
    public static bool HasLegacyElevatedStartupTask() => TaskExists(StartupTaskName);
    ...
    public static void MigrateLegacyStartup()
    {
        ...
        DeleteTask(StartupTaskName);
        EnsureElevatedLauncher();
        TryTerminateOtherInstalledInstances(TimeSpan.FromSeconds(6), out _);
    }
    ...
    public static bool TryRunElevated(IReadOnlyList<string> arguments, out string error)
    {
        ...
        try
        {
            ...
            dynamic task = root.GetTask(LauncherTaskName);
    ...
    static bool AppInstanceExists()
    {
        try
        {
            if (!Mutex.TryOpenExisting(App.InstanceMutexName, out var existing))
                return false;
            existing.Dispose();
            return true;
        }
    ...
    public static string? GetRegisteredCommand() => GetUserRunEntry() ?? GetTaskCommand(StartupTaskName);
    ...
    static string? GetTaskCommand(string name)
    {
        try
        {
            dynamic service = ConnectService();
            dynamic root = service.GetFolder("\\");
            dynamic task = root.GetTask(name);
            dynamic action = task.Definition.Actions.Item(1);
            return $"\"{(string)action.Path}\" {(string)action.Arguments}".TrimEnd();
        }
        catch { return null; }
    }
    ...
    static void SetUserRunEntry(bool enabled)
    {
        using var key = Registry.CurrentUser.CreateSubKey(RunKey);
        if (key == null)
            return;
        if (enabled)
            key.SetValue("RELYR", BuildCurrentCommand(), RegistryValueKind.String);
        else
            key.DeleteValue("RELYR", false);
    }
    static string? GetUserRunEntry()
    {
        try
        {
            using var key = Registry.CurrentUser.OpenSubKey(RunKey);
            return key?.GetValue("RELYR") as string;
        }
        catch { return null; }
    }
    static void RequireElevated()
    {
        if (!IsProcessElevated())
            throw new UnauthorizedAccessException("この設定には管理者モードが必要です。RELYRをインストールし直してください。");
    }
}
...
internal static class LegacyKeyRemapService
{
    ...
    {
        if (!config.CapsLockRemapPendingRestart || config.CapsLockRemapChangedAtUtcTicks <= 0)
            return false;
        long now = (utcNow ?? DateTime.UtcNow).Ticks;
        long elapsedTicks = Math.Max(0, now - config.CapsLockRemapChangedAtUtcTicks);
        long uptimeTicks = Math.Max(0, uptimeMilliseconds ?? Environment.TickCount64) * TimeSpan.TicksPerMillisecond;
    ...
    internal static byte[] UpdateCapsLockToF13(byte[]? current, bool enabled)
    {
        ...
        if (enabled)
            entries.Add(((uint)CapsLockScanCode << 16) | F13ScanCode);
        if (entries.Count == 0)
            return [];
        byte[] result = new byte[12 + (entries.Count + 1) * 4];
        BitConverter.GetBytes(entries.Count + 1).CopyTo(result, 8);
        for (int i = 0; i < entries.Count; i++)
            BitConverter.GetBytes(entries[i]).CopyTo(result, 12 + i * 4);
        return result;
    }
    ...
    static List<uint> Entries(byte[] bytes)
    {
        ...
        for (int i = 0; i < count; i++)
            result.Add(BitConverter.ToUInt32(bytes, 12 + i * 4));
        return result;
    }
