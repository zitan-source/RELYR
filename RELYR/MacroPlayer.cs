...
{
    sealed class ExecutionState{public int ExecutedSteps;}
    ...
    static async Task<MacroPlaybackResult> RunAfter(Task previous,MacroDefinition macro,CancellationTokenSource source,AppConfig? config,Action<string>? switchProfile,IntPtr? preferredActiveWindow)
    {
        ...
        {
            InputEngine.ReleaseAll();lock(gate){if(ReferenceEquals(current,source))current=null;}source.Dispose();
        }
        try{PlaybackFinished?.Invoke(result);}catch{}
        return result;
    }
    ...
    {
        if(depth>=MaximumNestedDepth)throw new InvalidOperationException($"マクロの呼び出しが{MaximumNestedDepth}段を超えました。");
        string identity=string.IsNullOrWhiteSpace(macro.Id)?macro.Name:macro.Id;
        if(!chain.Add(identity))throw new InvalidOperationException($"マクロ「{macro.Name}」が自分自身を繰り返し呼び出しています。");
        try
        {
            foreach(var step in macro.Steps)
            {
                token.ThrowIfCancellationRequested();if(++state.ExecutedSteps>MaximumExecutedSteps)throw new InvalidOperationException("マクロの実行手順が多すぎるため、安全のため停止しました。");
                if(step.DelayMs>0)await Task.Delay(Math.Clamp(step.DelayMs,0,600000),token).ConfigureAwait(false);token.ThrowIfCancellationRequested();
                if(step.RecordedActionKind is { } kind)
                {
                    if(kind==ActionKind.Macro)
                    {
                        if(config==null)throw new InvalidOperationException("呼び出すマクロの設定を読み込めません。");
                    ...
                    else
                    {
                        ...
                        if(!executor.Execute(new Mapping{Kind=kind,Value=step.RecordedActionValue,Layer="通常"},"Recorded",out string value))throw new InvalidOperationException("アクションを実行できません: "+value);
                    }
                }
