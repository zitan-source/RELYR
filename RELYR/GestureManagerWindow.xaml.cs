...
public partial class GestureManagerWindow:Window
{
    ...
    {
        loading=true;
        GestureList.ItemsSource=null;GestureList.ItemsSource=gestures;
        GestureList.SelectedItem=gestures.FirstOrDefault(x=>x.Name==(selectedName??SelectedGesture?.Name))??gestures.FirstOrDefault();
        loading=false;RefreshEditor();
    }

    void GestureSelectionChanged(object sender,SelectionChangedEventArgs e){if(!loading)RefreshEditor();}

    void RefreshEditor()
    {
        var gesture=SelectedGesture;GestureEditor.IsEnabled=gesture!=null;GestureTitle.Text=gesture?.Name??"ジェスチャーを追加してください";
    ...
    {
        string? name=PromptName("ジェスチャーを追加","新しいジェスチャー名",UniqueName("新しいジェスチャー"));if(string.IsNullOrWhiteSpace(name))return;
        if(gestures.Any(x=>x.Name.Equals(name,StringComparison.OrdinalIgnoreCase))){ShowStatus("同じ名前のジェスチャーがあります。",true);return;}
        gestures.Add(new GestureDefinition{Name=name});RefreshGestures(name);ShowStatus("ジェスチャーを追加しました。");
    }

    void RenameGesture_Click(object sender,RoutedEventArgs e)
    {
        var gesture=SelectedGesture;if(gesture==null)return;string old=gesture.Name;
        string? name=PromptName("ジェスチャー名を変更","新しい名前",old);if(string.IsNullOrWhiteSpace(name)||name==old)return;
        if(gestures.Any(x=>!ReferenceEquals(x,gesture)&&x.Name.Equals(name,StringComparison.OrdinalIgnoreCase))){ShowStatus("同じ名前のジェスチャーがあります。",true);return;}
        gesture.Name=name;RenameReferences(profiles,old,name);RefreshGestures(name);ShowStatus("名前と割り当てからの参照を変更しました。");
    }

    void DeleteGesture_Click(object sender,RoutedEventArgs e)
    {
        var gesture=SelectedGesture;if(gesture==null)return;
        if(AppDialog.Show(this,$"「{gesture.Name}」を削除しますか？\nこのジェスチャーを参照している長押し割り当ても解除されます。","ジェスチャーを削除",MessageBoxButton.YesNo,MessageBoxImage.Warning)!=MessageBoxResult.Yes)return;
        string deleted=gesture.Name;gestures.Remove(gesture);ClearReferences(profiles,deleted);RefreshGestures();ShowStatus("ジェスチャーと参照していた長押し割り当てを削除しました。");
    }

    void SelectAction_Click(object sender,RoutedEventArgs e)
    {
        if(SelectedGesture==null||sender is not System.Windows.Controls.Button{Tag:string slot} button)return;
    ...
    void ChooseAction(string slot,ActionKind requestedKind)
    {
        ...
        ActionKind kind=requestedKind;string? value=requestedKind switch
        {
        ...
        SetAction(SelectedGesture,slot,kind,value);RefreshEditor();ShowStatus($"{SlotLabel(slot)}の動作を設定しました。");
    }
    ...
    string? PickNamedValue(string title,string heading,IEnumerable<string> values)
    {
        ...
        if(choices.Count==0){ShowStatus("選択できる項目がありません。",true);return null;}
        ...
        var grid=new Grid{Margin=new Thickness(22)};grid.RowDefinitions.Add(new(){Height=GridLength.Auto});grid.RowDefinitions.Add(new());grid.RowDefinitions.Add(new(){Height=GridLength.Auto});
        var label=new TextBlock{Text=heading,FontSize=18,FontWeight=FontWeights.SemiBold,Margin=new Thickness(0,0,0,10)};grid.Children.Add(label);
        var list=new System.Windows.Controls.ListBox{ItemsSource=choices,Background=ThemeService.Brush("InputBackground"),Foreground=ThemeService.Brush("PrimaryText"),BorderBrush=ThemeService.Brush("BorderBrush")};Grid.SetRow(list,1);grid.Children.Add(list);
        ...
        list.SelectionChanged+=(_,_)=>ok.IsEnabled=list.SelectedItem!=null;list.MouseDoubleClick+=(_,_)=>{if(list.SelectedItem!=null)dialog.DialogResult=true;};
        cancel.Click+=(_,_)=>dialog.DialogResult=false;ok.Click+=(_,_)=>dialog.DialogResult=true;buttons.Children.Add(cancel);buttons.Children.Add(ok);Grid.SetRow(buttons,2);grid.Children.Add(buttons);dialog.Content=grid;
        return dialog.ShowDialog()==true?list.SelectedItem as string:null;
    }

    void ClearAction_Click(object sender,RoutedEventArgs e)
    {
        if(SelectedGesture==null||sender is not System.Windows.Controls.Button{Tag:string slot})return;
        SetAction(SelectedGesture,slot,ActionKind.None,"");RefreshEditor();ShowStatus($"{SlotLabel(slot)}の動作をクリアしました。");
    }
    ...
    static void SetAction(GestureDefinition gesture,string slot,ActionKind kind,string value)
    {
        switch(slot)
        {
            case "Up":gesture.UpKind=kind;gesture.UpValue=value;break;
            case "Down":gesture.DownKind=kind;gesture.DownValue=value;break;
            case "Left":gesture.LeftKind=kind;gesture.LeftValue=value;break;
            case "Right":gesture.RightKind=kind;gesture.RightValue=value;break;
            case "Center":gesture.CenterKind=kind;gesture.CenterValue=value;break;
        }
    }
    ...
    internal static void RenameReferences(IEnumerable<Profile> profiles,string oldName,string newName)
    {
        ...
        {
            if(map.Kind==ActionKind.Gesture&&map.Value.Equals(oldName,StringComparison.OrdinalIgnoreCase))map.Value=newName;
            if(map.LongPressKind==ActionKind.Gesture&&map.LongPressValue.Equals(oldName,StringComparison.OrdinalIgnoreCase))map.LongPressValue=newName;
        }
    }
    ...
    internal static void ClearReferences(IEnumerable<Profile> profiles,string deletedName)
    {
        foreach(var map in profiles.SelectMany(x=>x.Mappings))
        {
            if(map.Kind==ActionKind.Gesture&&map.Value.Equals(deletedName,StringComparison.OrdinalIgnoreCase)){map.Kind=ActionKind.None;map.Value="";}
            if(map.LongPressKind==ActionKind.Gesture&&map.LongPressValue.Equals(deletedName,StringComparison.OrdinalIgnoreCase)){map.LongPressKind=ActionKind.None;map.LongPressValue="";}
        }
    }
    ...
    string UniqueName(string basis){string name=basis;int number=2;while(gestures.Any(x=>x.Name.Equals(name,StringComparison.OrdinalIgnoreCase)))name=$"{basis} {number++}";return name;}
    ...
    string? PromptName(string title,string label,string initial,bool trim=true)
    {
        ...
        var panel=new StackPanel{Margin=new Thickness(24)};panel.Children.Add(new TextBlock{Text=label,FontSize=15,FontWeight=FontWeights.SemiBold,Margin=new Thickness(0,0,0,9)});var box=new System.Windows.Controls.TextBox{Text=initial,FontSize=15,Height=42,Padding=new Thickness(10,8,10,8),Background=ThemeService.Brush("InputBackground"),Foreground=ThemeService.Brush("PrimaryText"),BorderBrush=ThemeService.Brush("BorderBrush")};panel.Children.Add(box);
        var buttons=new StackPanel{Orientation=System.Windows.Controls.Orientation.Horizontal,HorizontalAlignment=System.Windows.HorizontalAlignment.Right,Margin=new Thickness(0,18,0,0)};var cancel=new System.Windows.Controls.Button{Content="キャンセル",Width=112,Height=40,Margin=new Thickness(6,0,0,0),Style=(Style)System.Windows.Application.Current.FindResource("AppButtonStyle")};var ok=new System.Windows.Controls.Button{Content="決定",Width=112,Height=40,Margin=new Thickness(6,0,0,0),Style=(Style)System.Windows.Application.Current.FindResource("AccentAppButtonStyle"),IsDefault=true};cancel.Click+=(_,_)=>dialog.DialogResult=false;ok.Click+=(_,_)=>dialog.DialogResult=true;buttons.Children.Add(cancel);buttons.Children.Add(ok);panel.Children.Add(buttons);dialog.Content=panel;dialog.Loaded+=(_,_)=>{box.Focus();box.SelectAll();};return dialog.ShowDialog()==true?(trim?box.Text.Trim():box.Text):null;
    }
