...
public partial class ActionPickerWindow : Window
{
    ...
    public ActionPickerWindow(IEnumerable<Profile>? profiles = null, string keyboardLayout = "JIS", IEnumerable<GestureDefinition>? gestures = null, bool includeGestures = false, string? initialMajorCategory = null, IEnumerable<DeckLayoutDefinition>? deckLayouts = null)
    {
        ...
        RefreshSearch();
        if (initialMajorCategory != null && MajorCategoryList.Items.Contains(initialMajorCategory))
            MajorCategoryList.SelectedItem = initialMajorCategory;
    }
    ...
    void RefreshSearch()
    {
        ...
        filtered = [.. (text.Length == 0 ? allActions : allActions.Where(x => new[] { x.MajorCategory, x.Category, x.Name, x.Description, x.Value }.Any(value => value.Contains(text, StringComparison.OrdinalIgnoreCase))))];
        ...
        if (MajorCategoryList.SelectedItem == null)
        {
    ...
    void MajorCategoryChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        if (MajorCategoryList.SelectedItem is not string major)
        {
            CategoryList.ItemsSource = null;
            ActionList.ItemsSource = null;
            return;
    ...
    void RefreshActions()
    {
        if (MajorCategoryList.SelectedItem is not string major || CategoryList.SelectedItem is not string category)
        {
            ActionList.ItemsSource = null;
            ResultCount.Text = "0件";
            return;
        ...
        ResultCount.Text = $"{actions.Count}件";
    }
    ...
    internal static List<CatalogAction> ActionsForCategory(IEnumerable<CatalogAction> actions, string major, string category)
        => [.. actions.Where(x => x.MajorCategory == major && (category == AllCategories || x.Category == category))];
    ...
    void ActionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        ClockSettingsButton.Visibility = ActionList.SelectedItem is CatalogAction { Value: OverlayService.ClockAction }
    ...
    {
#if !PRODUCTION_PUBLISH
        if (OpenClockSettingsForTest is { } test)
        {
            test(this);
            return;
        }
        ...
        for (Window? owner = Owner; owner != null; owner = owner.Owner)
            ...
            {
                main.OpenSettingsFrom(this, "Overlay");
                return;
            }
    }

    void ActionDoubleClick(object sender, MouseButtonEventArgs e)
    {
    ...
    void Accept(CatalogAction action)
    {
        result = action;
        DialogResult = true;
        Close();
    ...
    void OpenKeypad_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            if (CustomShortcutBox.Text == value)
                return;
    ...
    void UseShortcut_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (value.Length > 0)
            Accept(new CatalogAction("任意のショートカット", "任意のショートカット", "キーパッドまたは手入力で指定したキー操作です", ActionKind.Shortcut, value));
    }
