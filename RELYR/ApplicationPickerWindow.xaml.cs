...
public partial class ApplicationPickerWindow:Window
{
    ...
    async void ApplicationPickerWindow_Loaded(object sender,RoutedEventArgs e)
    {
        ...
        if(IsLoaded)SetApplications(discovered);
    }

    void SetApplications(IEnumerable<InstalledApplicationInfo> items)
    {
        applications=items.GroupBy(x=>x.LaunchPath,StringComparer.OrdinalIgnoreCase).Select(x=>x.OrderBy(y=>y.Name.Length).First()).OrderBy(x=>x.Name,StringComparer.CurrentCultureIgnoreCase).ToList();
        RefreshList();
    }
    ...
    void RefreshList()
    {
        ...
        var filtered=string.IsNullOrEmpty(query)?applications:applications.Where(x=>x.Name.Contains(query,StringComparison.CurrentCultureIgnoreCase)||x.LaunchPath.Contains(query,StringComparison.OrdinalIgnoreCase)).ToList();
        ApplicationList.ItemsSource=filtered;ResultCount.Text=$"{filtered.Count}件";EmptyMessage.Text=applications.Count==0?"起動できるアプリが見つかりませんでした":"一致するアプリがありません";EmptyMessage.Visibility=filtered.Count==0?Visibility.Visible:Visibility.Collapsed;
    }
    ...
    static void ReadAppPaths(List<InstalledApplicationInfo> found,RegistryHive hive,RegistryView view)
    {
        try
        {
            ...
            foreach(string name in root.GetSubKeyNames())
            {
                ...
                if(app!=null)Add(found,Path.GetFileNameWithoutExtension(name),NormalizeExecutablePath(app.GetValue(null) as string),"インストール済みアプリ");
            }
        }
        catch(UnauthorizedAccessException){}catch(System.Security.SecurityException){}catch(IOException){}
    }
    ...
    static void ReadUninstallEntries(List<InstalledApplicationInfo> found,RegistryHive hive,RegistryView view)
    {
        try
        {
            ...
            foreach(string keyName in root.GetSubKeyNames())
            {
                ...
                if(app!=null)Add(found,app.GetValue("DisplayName") as string,NormalizeExecutablePath(app.GetValue("DisplayIcon") as string),"Windowsに登録されたアプリ");
            }
        }
        catch(UnauthorizedAccessException){}catch(System.Security.SecurityException){}catch(IOException){}
    }
    ...
    {
        if(string.IsNullOrWhiteSpace(name)||string.IsNullOrWhiteSpace(path)||name.Contains("uninstall",StringComparison.OrdinalIgnoreCase)||name.Contains("アンインストール",StringComparison.OrdinalIgnoreCase))return;
    ...
    static string? NormalizeExecutablePath(string? value)
    {
        ...
        if(text.StartsWith('"')){int end=text.IndexOf('"',1);if(end>1)text=text[1..end];}
        else{int end=text.IndexOf(".exe",StringComparison.OrdinalIgnoreCase);if(end>=0)text=text[..(end+4)];}
    ...
    static IEnumerable<string> EnumerateFilesSafe(string root)
    {
        ...
        while(pending.Count>0)
        {
            ...
            string[] files=[];try{files=Directory.GetFiles(folder);}catch(UnauthorizedAccessException){}catch(IOException){}
            foreach(string file in files)yield return file;
            string[] children=[];try{children=Directory.GetDirectories(folder);}catch(UnauthorizedAccessException){}catch(IOException){}
