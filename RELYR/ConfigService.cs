...
public sealed class ConfigService
{
    ...
    static string DeckMappingSignature(IEnumerable<Mapping> mappings)=>string.Join("\n",mappings.Where(x=>DeckPanelLayout.IsInputName(x.Input)).OrderBy(x=>DeckPanelLayout.SlotNumber(x.Input)).Select(x=>$"{x.Input}\u001f{x.Kind}\u001f{x.Value}\u001f{x.LongPressKind}\u001f{x.LongPressValue}\u001f{x.LongPressMs}\u001f{x.Application}\u001f{x.Description}"));
    ...
    static Mapping CloneMapping(Mapping x)=>new(){Input=x.Input,Kind=x.Kind,Value=x.Value,LongPressMs=x.LongPressMs,LongPressValue=x.LongPressValue,LongPressKind=x.LongPressKind,DragValue=x.DragValue,DragEndValue=x.DragEndValue,Application=x.Application,Layer=x.Layer,Description=x.Description};

    static void NormalizeMappings(IEnumerable<Mapping> mappings,int originalVersion)
    {
        foreach(var map in mappings)
        {
            map.Input??="";map.Value??="";map.LongPressValue??="";map.DragValue??="";map.DragEndValue??="";map.Application??="";map.Layer??="通常";map.Description??="";
