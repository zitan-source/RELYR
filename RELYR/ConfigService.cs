...
public sealed class ConfigService
{
    ...
    static AppConfig Normalize(AppConfig value)
    {
        ...
        NormalizeGestures(value.Gestures, originalVersion, value.LockCursorDuringGesture, value.GestureThresholdPixels);
    ...
    static void NormalizeGestures(IEnumerable<GestureDefinition> gestures, int originalVersion, bool legacyLockCursorDuringGesture, int legacyGestureThresholdPixels)
    {
        foreach (var gesture in gestures)
        {
            ...
            if (originalVersion < PerGestureThresholdMigrationVersion)
                gesture.GestureThresholdPixels = legacyGestureThresholdPixels;
            gesture.GestureThresholdPixels = Math.Clamp(gesture.GestureThresholdPixels, 3, 100);
            gesture.UpValue ??= "";
            gesture.DownValue ??= "";
            gesture.LeftValue ??= "";
            gesture.RightValue ??= "";
    ...
    static string DeckMappingSignature(IEnumerable<Mapping> mappings) => string.Join("\n", mappings.Where(x => DeckPanelLayout.IsInputName(x.Input)).OrderBy(x => DeckPanelLayout.SlotNumber(x.Input)).Select(x => $"{x.Input}\u001f{x.Kind}\u001f{x.Value}\u001f{x.LongPressKind}\u001f{x.LongPressValue}\u001f{x.LongPressMs}\u001f{x.Application}\u001f{x.Description}\u001f{x.DeckColor}\u001f{x.DeckFilePath}\u001f{x.DeckIcon}\u001f{x.DeckIconPath}\u001f{x.DeckIconAutoAssigned}\u001f{x.DeckIconHidden}\u001f{x.DeckMonitor}"));
