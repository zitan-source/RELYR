...
internal static class ProfileSwitchRuntimeTest
{
    ...
    internal static async Task<int> RunAsync()
    {
        ...
        try
        {
            ...
            var automaticProfiles = saved.Profiles.Skip(1)
                ...
                .ToList();
            if (automaticProfiles.Count == 0)
                throw new InvalidOperationException("自動切替が有効なプロファイルがありません。");
            ...
            foreach (var item in profileHosts)
            {
                ...
                for (int cycle = 1; cycle <= 3; cycle++)
                {
                    ...
                    string[] candidates = [.. ConditionMatcher.ProcessesUnderCursor()];
    ...
    static string DescribeCursorWindow()
    {
        ...
        try
        {
            using var value = Process.GetProcessById((int)pid);
    ...
    static async Task<bool> WaitForProfile(MainWindow window, string expected, int timeoutMs)
    {
        ...
        {
            if (window.AppliedProfileNameForTest.Equals(expected, StringComparison.OrdinalIgnoreCase))
                return true;
            await Task.Delay(100);
        }
