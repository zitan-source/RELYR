...
- Use `RELYR/artifacts/production` only for the current production installer and checksum.
- Keep the current installer and its matching `.sha256` file in `artifacts/production`.
...
- Confirm that the output directory is `artifacts/production` and that it contains only the current installer and its matching `.sha256` file after generation.
