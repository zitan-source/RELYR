...
if($installerText -notmatch '(?im)^ShowLanguageDialog=yes\s*if($installerText -match '(?is)taskkill(?:\.exe)?.*?/IM\s+(?:RELYR|InputCustomizer)\.exe'){
...
foreach($language in $termsByLanguage.Keys){
  ...
  if($installerText -notmatch ('(?im)^Source:\s*"'+$escapedPath+'";.*DestName:\s*"TERMS\.txt".*Languages:\s*'+[regex]::Escape($language)+'\s*if($installerText -match '(?is)taskkill(?:\.exe)?.*?/IM\s+(?:RELYR|InputCustomizer)\.exe'){
  throw "Unsafe global RELYR process termination was found in installer.iss"
}
$uninstallRun=[regex]::Match($installerText,'(?ms)^\[UninstallRun\]\s*(.*?)(?=^\[|\z)').Groups[1].Value
if($uninstallRun -match '(?i)--shutdown-existing|taskkill'){
