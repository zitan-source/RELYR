...
- The Deck editor keeps its command bar stable while dimensions change. Its default body belongs entirely to the Deck preview; settings appear only on demand in one `Deckをカスタマイズ` drawer with Layout, Appearance, and Behavior tabs. The drawer occupies a bounded column between preview and inspector when wide and stacks above the preview with hysteresis when narrow. Closing it returns all body space to the preview. The Deck name is a plain inline-editable heading, the application-level Save action stays in the top toolbar, and compact widths replace secondary command labels with tooltip- and automation-named icons rather than wrapping or clipping them. A one-row 18-column or 18-row one-column Deck fits completely when the editor has sufficient space; only genuinely constrained single-axis layouts and dense grids such as 18x18 scroll inside the preview.
