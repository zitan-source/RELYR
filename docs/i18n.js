(() => {
  ...
  const githubPrivacyLink = '<a href="https://docs.github.com/en/site-policy/privacy-policies/github-general-privacy-statement">GitHub General Privacy Statement</a>';
  const legalTranslations = {
    'ja-JP': ['プライバシー', 'このWebサイトについて', 'このサイトはRELYRを紹介する静的なWebサイトです。RELYR独自のアクセス解析、広告、入力フォームは設置していません。表示言語だけをブラウザーのローカルストレージに保存します。ページ表示時には、ホスティング事業者とGoogle Fontsが通常の接続情報を処理する場合があります。', '外部サイトへのリンク', 'ダウンロード、ソースコード、支援ページでは、GitHubまたはKo-fiのプライバシーポリシーが適用されます。', 'RELYRアプリについて', 'RELYRはキー・マウスの入力内容、割り当て、マクロ、設定、ローカルファイル名を作者または外部サーバーへ送信しません。設定とマクロは利用者のPC内へ保存します。', 'アップデート確認', '最新版の確認が有効な場合、RELYRはGitHub ReleasesへHTTPSで接続します。GitHubはIPアドレス、リクエスト時刻、User-Agentなど通常の接続情報を同社の規約に従って処理する場合があります。この通信に入力内容、割り当て、マクロ、設定、固有の端末IDは含めません。自動確認はRELYRの設定で無効にできます。更新のダウンロード時もGitHubへ接続します。詳細は' + githubPrivacyLink + 'をご確認ください。', '← RELYRのホームへ戻る'],
    'en-US': ['Privacy', 'About this website', 'This static website introduces RELYR. It has no RELYR-operated analytics, advertising, or input forms. It stores only your display-language preference in browser local storage. The hosting provider and Google Fonts may process ordinary connection data when a page loads.', 'Links to external websites', 'GitHub or Ko-fi privacy policies apply to download, source code, and support pages.', 'About the RELYR app', 'RELYR does not send keyboard or mouse input content, mappings, macros, settings, or local file names to its authors or any external server. Settings and macros remain on your PC.', 'Update checks', 'When update checking is enabled, RELYR connects to GitHub Releases over HTTPS. GitHub may process ordinary connection data such as your IP address, request time, and User-Agent under its policy. The request contains no input content, mappings, macros, settings, or unique device ID. Automatic checks can be disabled in RELYR settings. Downloading an update also connects to GitHub. See the ' + githubPrivacyLink + '.', '← Back to RELYR'],
    'zh-CN': ['隐私', '关于本网站', '这是介绍 RELYR 的静态网站。网站不使用由 RELYR 运营的分析、广告或输入表单，仅在浏览器本地存储中保存显示语言偏好。页面加载时，托管服务商与 Google Fonts 可能处理常规连接信息。', '外部网站链接', '下载、源代码与支持页面适用 GitHub 或 Ko-fi 的隐私政策。', '关于 RELYR 应用', 'RELYR 不会向作者或任何外部服务器发送键盘或鼠标输入内容、映射、宏、设置或本地文件名。设置和宏保存在您的电脑中。', '更新检查', '启用更新检查时，RELYR 会通过 HTTPS 连接 GitHub Releases。GitHub 可能依其政策处理 IP 地址、请求时间和 User-Agent 等常规连接信息。请求不包含输入内容、映射、宏、设置或唯一设备 ID。您可在 RELYR 设置中关闭自动检查。下载更新时也会连接 GitHub。详情请参阅 ' + githubPrivacyLink + '。', '← 返回 RELYR'],
    'zh-TW': ['隱私權', '關於本網站', '這是介紹 RELYR 的靜態網站。網站不使用由 RELYR 營運的分析、廣告或輸入表單，只在瀏覽器本機儲存空間保存顯示語言偏好。頁面載入時，託管服務商與 Google Fonts 可能處理一般連線資訊。', '外部網站連結', '下載、原始碼與支援頁面適用 GitHub 或 Ko-fi 的隱私權政策。', '關於 RELYR 應用程式', 'RELYR 不會向作者或任何外部伺服器傳送鍵盤或滑鼠輸入內容、對應、巨集、設定或本機檔案名稱。設定與巨集保存在您的電腦中。', '更新檢查', '啟用更新檢查時，RELYR 會透過 HTTPS 連線至 GitHub Releases。GitHub 可能依其政策處理 IP 位址、請求時間與 User-Agent 等一般連線資訊。請求不包含輸入內容、對應、巨集、設定或唯一裝置 ID。您可在 RELYR 設定中關閉自動檢查。下載更新時也會連線至 GitHub。詳情請參閱 ' + githubPrivacyLink + '。', '← 返回 RELYR'],
    'ko-KR': ['개인정보 보호', '이 웹사이트에 대하여', 'RELYR를 소개하는 정적 웹사이트입니다. RELYR가 운영하는 분석, 광고 또는 입력 양식은 없으며 표시 언어 설정만 브라우저 로컬 저장소에 저장합니다. 페이지를 불러올 때 호스팅 제공자와 Google Fonts가 일반적인 연결 정보를 처리할 수 있습니다.', '외부 사이트 링크', '다운로드, 소스 코드와 후원 페이지에는 GitHub 또는 Ko-fi의 개인정보 처리방침이 적용됩니다.', 'RELYR 앱에 대하여', 'RELYR는 키보드·마우스 입력 내용, 매핑, 매크로, 설정 또는 로컬 파일 이름을 작성자나 외부 서버로 보내지 않습니다. 설정과 매크로는 사용자의 PC에 저장됩니다.', '업데이트 확인', '업데이트 확인이 켜져 있으면 RELYR는 HTTPS로 GitHub Releases에 연결합니다. GitHub는 정책에 따라 IP 주소, 요청 시각, User-Agent와 같은 일반 연결 정보를 처리할 수 있습니다. 요청에는 입력 내용, 매핑, 매크로, 설정 또는 고유 기기 ID가 포함되지 않습니다. 자동 확인은 RELYR 설정에서 끌 수 있습니다. 업데이트 다운로드 시에도 GitHub에 연결합니다. 자세한 내용은 ' + githubPrivacyLink + '를 참조하세요.', '← RELYR로 돌아가기'],
    'fr-FR': ['Confidentialité', 'À propos de ce site', 'Ce site statique présente RELYR. Il ne comporte ni analyse gérée par RELYR, ni publicité, ni formulaire. Seule la préférence de langue est conservée dans le stockage local du navigateur. L’hébergeur et Google Fonts peuvent traiter les données de connexion ordinaires au chargement.', 'Liens externes', 'Les politiques de confidentialité de GitHub ou Ko-fi s’appliquent aux pages de téléchargement, de code source et de soutien.', 'À propos de l’application RELYR', 'RELYR n’envoie aux auteurs ni à aucun serveur externe le contenu du clavier ou de la souris, les affectations, les macros, les réglages ou les noms de fichiers locaux. Les réglages et macros restent sur votre PC.', 'Recherche de mises à jour', 'Lorsque cette fonction est activée, RELYR se connecte à GitHub Releases via HTTPS. GitHub peut traiter des données de connexion ordinaires telles que l’adresse IP, l’heure de la requête et le User-Agent conformément à sa politique. La requête ne contient aucun contenu de saisie, affectation, macro, réglage ou identifiant unique d’appareil. La vérification automatique peut être désactivée dans RELYR. Le téléchargement se connecte également à GitHub. Consultez la ' + githubPrivacyLink + '.', '← Retour à RELYR'],
    'de-DE': ['Datenschutz', 'Über diese Website', 'Diese statische Website stellt RELYR vor. Sie verwendet keine von RELYR betriebene Analyse, Werbung oder Eingabeformulare. Nur die Anzeigesprache wird im lokalen Browserspeicher gespeichert. Beim Laden können Hosting-Anbieter und Google Fonts übliche Verbindungsdaten verarbeiten.', 'Externe Links', 'Für Download-, Quellcode- und Supportseiten gelten die Datenschutzrichtlinien von GitHub oder Ko-fi.', 'Über die RELYR-App', 'RELYR sendet Tastatur- oder Mauseingaben, Zuordnungen, Makros, Einstellungen oder lokale Dateinamen weder an die Autoren noch an externe Server. Einstellungen und Makros bleiben auf deinem PC.', 'Update-Prüfung', 'Wenn die Update-Prüfung aktiviert ist, verbindet sich RELYR per HTTPS mit GitHub Releases. GitHub kann gemäß seiner Richtlinie übliche Verbindungsdaten wie IP-Adresse, Anfragezeit und User-Agent verarbeiten. Die Anfrage enthält keine Eingaben, Zuordnungen, Makros, Einstellungen oder eindeutige Geräte-ID. Die automatische Prüfung lässt sich in RELYR deaktivieren. Auch der Download verbindet sich mit GitHub. Siehe ' + githubPrivacyLink + '.', '← Zurück zu RELYR'],
    'es-ES': ['Privacidad', 'Acerca de este sitio', 'Este sitio estático presenta RELYR. No incluye analítica gestionada por RELYR, publicidad ni formularios. Solo guarda la preferencia de idioma en el almacenamiento local del navegador. El proveedor de alojamiento y Google Fonts pueden tratar datos de conexión normales al cargar la página.', 'Enlaces externos', 'Las políticas de privacidad de GitHub o Ko-fi se aplican a las páginas de descarga, código fuente y soporte.', 'Acerca de la aplicación RELYR', 'RELYR no envía a sus autores ni a ningún servidor externo el contenido del teclado o ratón, las asignaciones, macros, ajustes o nombres de archivos locales. Los ajustes y macros permanecen en tu PC.', 'Comprobación de actualizaciones', 'Cuando está activada, RELYR se conecta a GitHub Releases mediante HTTPS. GitHub puede tratar datos de conexión normales, como la dirección IP, la hora de la solicitud y el User-Agent, conforme a su política. La solicitud no contiene entradas, asignaciones, macros, ajustes ni un identificador único del dispositivo. La comprobación automática puede desactivarse en RELYR. La descarga también conecta con GitHub. Consulta la ' + githubPrivacyLink + '.', '← Volver a RELYR']
  };
  ...
  const interfaceTranslations = {
    'ja-JP': { layers: ['デフォルト', 'Space', 'CapsLock', '右クリック', '進む / 戻る', 'タスクバー'], footer: ['リリース', 'ソース', 'サポート', 'プライバシー', 'ライセンス'] },
    'en-US': { layers: ['Default', 'Space', 'CapsLock', 'Right Click', 'Forward / Back', 'Taskbar'], footer: ['Releases', 'Source', 'Support', 'Privacy', 'License'] },
    'zh-CN': { layers: ['默认', 'Space', 'CapsLock', '右键', '前进 / 后退', '任务栏'], footer: ['版本', '源代码', '支持', '隐私', '许可证'] },
    'zh-TW': { layers: ['預設', 'Space', 'CapsLock', '右鍵', '前進 / 返回', '工作列'], footer: ['版本', '原始碼', '支援', '隱私權', '授權'] },
    'ko-KR': { layers: ['기본', 'Space', 'CapsLock', '오른쪽 클릭', '앞으로 / 뒤로', '작업 표시줄'], footer: ['릴리스', '소스', '지원', '개인정보', '라이선스'] },
    'fr-FR': { layers: ['Par défaut', 'Espace', 'Verr. maj.', 'Clic droit', 'Suivant / Retour', 'Barre des tâches'], footer: ['Versions', 'Source', 'Assistance', 'Confidentialité', 'Licence'] },
    'de-DE': { layers: ['Standard', 'Leertaste', 'Feststelltaste', 'Rechtsklick', 'Vor / Zurück', 'Taskleiste'], footer: ['Versionen', 'Quellcode', 'Support', 'Datenschutz', 'Lizenz'] },
    'es-ES': { layers: ['Predeterminado', 'Espacio', 'Bloq Mayús', 'Clic derecho', 'Adelante / Atrás', 'Barra de tareas'], footer: ['Versiones', 'Código', 'Soporte', 'Privacidad', 'Licencia'] }
  };
  ...
  const applyLanguage = (code) => {
    ...
    if (one('.legal-shell') && legal) {
      ...
      list('.legal-shell h2 + p', [legal[2], legal[4], legal[6], legal[8]], true);
